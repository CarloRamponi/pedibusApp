...
class LineaPage extends StatefulWidget {
  ...
  LineaPage({Key key, String title, int id}) : title = title, id = id, super(key: key);

  @override
  _LineaPageState createState() => new _LineaPageState(id);
...
class _LineaPageState extends State<LineaPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return new FutureBuilder<Map<String, dynamic>>(
      future: Query.query("classes [linea] and id = '" + id.toString() + "'"),
      builder: (context, snapshot) {

        if (snapshot.hasData) {

          List<Widget> _fermate = new List<Widget>(snapshot.data['searchHits'][0]['data']['ita-IT']['fermate'].length);

          for(int i = 0; i < snapshot.data['searchHits'][0]['data']['ita-IT']['fermate'].length; i++){
            _fermate[i] = new ListTile(
              leading: new Icon(Icons.place),
              title: new Text(snapshot.data['searchHits'][0]['data']['ita-IT']['fermate'][i]['name']['ita-IT'].split("(")[0]),
              onTap: () {
                Navigator.of(context).push(
                    new MaterialPageRoute(builder: (context) => new FermataPage(
                      name: snapshot.data['searchHits'][0]['data']['ita-IT']['fermate'][i]['name']['ita-IT'].split("(")[0],
                      id: snapshot.data['searchHits'][0]['data']['ita-IT']['fermate'][i]['id'],
                    ))
                );
              },
            );
                    ...
                    children: <ExpansionPanel> [
                      ...
                      new ExpansionPanel(
                        ...
                        body: new Padding(
                          ...
                          child: new FutureBuilder<Map<String, dynamic>>(
                            future: Query.query("classes [assenza_volontario] and linea.codice = '" + snapshot.data['searchHits'][0]['data']['ita-IT']['codice'] + "' and data range [today, *]"),
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                      ...
                      new ExpansionPanel(
                        ...
                        body: new Padding(
                          padding: new EdgeInsets.all(20.0),
                          child: new FutureBuilder<Map<String, dynamic>>(
                            future: Query.query("classes [assenza_bambino] and linea.codice = '" + snapshot.data['searchHits'][0]['data']['ita-IT']['codice'] + "' and data range [today, *]"),
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                      ...
                      new ExpansionPanel(
                        ...
                        body: new Padding(
                          padding: new EdgeInsets.all(20.0),
                          child: new FutureBuilder<Map<String, dynamic>>(
                            future: Query.query("classes [disponibilita] and linea.id = '" + snapshot.data['searchHits'][0]['metadata']['id'].toString() + "' and dal range [*, now] and al range [now, *]"),
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
